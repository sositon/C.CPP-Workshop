 ...
 * @param vec The VLA.
 * @param expected_capacity The expected capacity_.
 * @param expected_size The expected size_.
 */
...
/**
 * Asserts that the given vector has the given size_.
 * @param vec The VLVector.
 * @param expectedSize The expected size_.
 *
 * MACRO EXPANSION:
 * if (vec.capacity_() != (expected_size)) {
 *      print error
 *      return 0;
 * }
 */
#define ASSERT_SIZE(vec, expected_size) if (vec.size() != (expected_size)) { \
        std::cerr << "Assertion failed: Incorrect size_. Expected: "#expected_size << "; Actual: " \
                  << vec.size() << "." << std::endl; \
        return 0; \
    }

/**
 * Asserts that the given vector has the given capacity_.
 * @param vec The VLVector.
 * @param expected_capacity The expected capacity_.
 *
 * MACRO EXPANSION:
 * if (vec.capacity_() != (expected_capacity)) {
 *      print error
 *      return 0;
 * }
 */
#define ASSERT_CAPACITY(vec, expected_capacity) if (vec.capacity() != (expected_capacity)) { \
        std::cerr << "Assertion failed: Incorrect capacity_. Expected: "#expected_capacity << "; Actual: " \
